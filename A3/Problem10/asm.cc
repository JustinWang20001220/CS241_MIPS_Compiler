...
int main() {
  ...
  for (auto tokenLine: tokenLines) {
    ...
    // check for whether it is a valid ID instruction
      ...
      else if (tokenLine[0].getLexeme() == "mult" || tokenLine[0].getLexeme() == "multu" 
      ...
      // check for lw, sw
      else if (tokenLine[0].getLexeme() == "sw" || tokenLine[0].getLexeme() == "lw") {
        if (tokenLine.size() == 7) {
          if (tokenLine[2].getKind() != Token::COMMA) {
            std::cerr << "ERROR: incorrect comma position/position" << std::endl;
            return 1;
          }
          else if (tokenLine[1].getKind() != Token::REG || tokenLine[5].getKind() != Token::REG) {
            std::cerr << "ERROR: incorrect reg position/position" << std::endl;
            return 1;
          }
          else if (tokenLine[4].getKind() != Token::LPAREN || tokenLine[6].getKind() != Token::RPAREN) {
            std::cerr << "ERROR: incorrect () position/position" << std::endl;
            return 1;
          }
          else if (tokenLine[3].getKind() != Token::INT && tokenLine[3].getKind() != Token::HEXINT) {
            std::cerr << "ERROR: incorrect INT/HEXINT position/position" << std::endl;
            return 1;
          }
          else {
            int reg1 = tokenLine[1].toNumber();
            int reg2 = tokenLine[5].toNumber();
            if (reg1 >= 0 && reg1 <= 31 && reg2 >= 0 && reg2 <= 31) {
              if ((tokenLine[3].getKind() == Token::INT && (tokenLine[3].toNumber() > INT16_MAX || tokenLine[3].toNumber() < INT16_MIN))
              || (tokenLine[3].getKind() == Token::HEXINT && tokenLine[3].toNumber() > 0xffff)) {
                std::cerr << "ERROR: out of range int/hexint" << std::endl;
                return 1;
              } 
              else {
                int sw_number = tokenLine[3].toNumber();
                if (sw_number < 0) {
                  if (tokenLine[0].getLexeme() == "lw") {
                    int instr = (35 << 26) | (reg2 << 21) | (reg1 << 16) | (sw_number & 0xFFFF);
                    print_instr(instr);
                  }
                  else {
                    int instr = (43 << 26) | (reg2 << 21) | (reg1 << 16) | (sw_number & 0xFFFF);
                    print_instr(instr);
                  }
                }
                else {
                  if (tokenLine[0].getLexeme() == "lw") {
                    int instr = (35 << 26) | (reg2 << 21) | (reg1 << 16) | sw_number ;
                    print_instr(instr);
                  }
                  else {
                    int instr = (43 << 26) | (reg2 << 21) | (reg1 << 16) | sw_number ;
                    print_instr(instr);
                  }
                }
              }
