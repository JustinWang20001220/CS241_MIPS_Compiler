    ...
    void generateCodeWithName(Tree::Node* n, string procedure_name) {
        if (n->rule == "main INT WAIN LPAREN dcl COMMA dcl RPAREN LBRACE dcls statements RETURN expr SEMI RBRACE" ) {
            if (procedure_name == "wain") {
                ...
                cout << "sub $30 , $30, $4 ; update stack pointer" << endl;
                push(31);
                if (getType(procedure_name, n->children[3]->children[1]->lexema) == "INT") {
                    cout << "add $2, $0, $0" << endl;
                }
                push(31);
                cout << "lis $5" << endl;
            ...
            if (LHS == "INT STAR" && RHS == "INT") {
                cout << "mult $3, $4" << endl;
                cout << "mflo $3" << endl;
            }
            else if (LHS == "INT" && RHS == "INT STAR") {
                cout << "mult $5, $4" << endl;
                cout << "mflo $5" << endl;
            }
            cout << "add $3, $5, $3" << endl;
            ...
            if (n->children[0]->rule == "lvalue STAR factor") {
                push(3);
                generateCodeWithName(n->children[0]->children[1], procedure_name);
                pop(5);
                cout << "sw $5, 0($3)" << endl;
            }
            else if (n->children[0]->rule == "lvalue ID") {
                cout << "sw $3, " << getOffset(procedure_name, variable_name) << "($29)" << endl;
                cout << "sub $30, $30, $4" << endl;
            }
        }
            ...
            generateCodeWithName(n->children[2], procedure_name);
            pop(5);
            if (checkTypeWithProcedureName(n->children[0], procedure_name) == "INT") {
                cout << "slt $3, $5, $3" << endl;
            }
            else {
        ...
        else if (n->rule == "factor AMP lvalue" && n->children[1]->rule == "lvalue ID") {
            cout << "lis $3" << endl;
            cout << ".word " << getOffset(procedure_name, n->children[1]->children[0]->lexema) << endl;
            cout << "add $3, $3, $29" << endl;
        }
        else if (n->rule == "factor AMP lvalue" && n->children[1]->rule == "lvalue STAR factor")  {
            generateCodeWithName(n->children[1]->children[1], procedure_name);
        }
        ...
        // A9 Q2
            ...
            generateCodeWithName(n->children[3], procedure_name);
            cout << "add $1, $3, $0" << endl;
            push(31);
            cout << "lis $5" << endl;
            cout << ".word new" << endl;
            ...
            int cur_delete_count = delete_count;
            this->delete_count += 1;
            generateCodeWithName(n->children[3], procedure_name);
            cout << "beq $3, $11, skipDelete" << cur_delete_count << endl;
            cout << "add $1, $3, $0" << endl; 
            push(31);
            cout << "lis $5" << endl;
            ...
            cout << "jalr $5" << endl;
            pop(31);
            cout << "skipDelete" << cur_delete_count << ':' << endl;
        }
        //////////////
        // A9 Q5
        else if (n->rule == "procedures procedure procedures") {
            generateCodeWithName(n->children[1], procedure_name);
            generateCodeWithName(n->children[0], procedure_name);
        }
        else if (n->rule == "procedure INT ID LPAREN params RPAREN LBRACE dcls statements RETURN expr SEMI RBRACE") {
            procedure_name = n->children[1]->lexema;
            if (n->children[3]->rule == "params paramlist") {
                int parameter_count = procedure_table[procedure_name].first.size();
                int adjusted_offset = parameter_count * 4;
                for (auto &s: procedure_table[procedure_name].second) {
                    s.second.second += adjusted_offset;
                }
            }
            cout << "F" << procedure_name << ":" << endl;
            cout << "sub $29, $30, $4 ; caller-saves old frame pointer" << endl;
            generateCodeWithName(n->children[6], procedure_name);
            push(1);
            push(2);
            push(5);
            push(6);
            push(7);                
            generateCodeWithName(n->children[7], procedure_name);
            generateCodeWithName(n->children[9], procedure_name);
            pop(7);
            pop(6);
            pop(5);
            ...
            push(31);
            cout << "lis $5" << endl;
            cout << ".word " << "F" << n->children[0]->lexema << endl;
            cout << "jalr $5" << endl;
            pop(31);
            pop(29);
        }
        ...
        else if (n->rule == "params paramlist") {
            return;
        }
        else if (n->rule == "paramlist dcl") {
            return;
        }
        else if (n->rule == "paramlist dcl COMMA paramlist") {
            return;
        }
        else if (n->rule == "arglist expr") {
            generateCodeWithName(n->children[0], procedure_name);
            push(3);
            push_count += 1;
        }
